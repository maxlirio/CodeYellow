...
// B: buy & place a barricade at the aimed spot
export function tryBuildBarricade() {
  if (!horde.active || !G.player || G.player.dead) return;
  if (G.run.gold < BARRICADE_COST) { addMsg(`Barricades cost ${BARRICADE_COST}g.`, 'bad'); return; }
  ...
  if (!hit) { addMsg('No clear spot there.', 'bad'); return; }
  const cx = Math.round(hit.x / 4), cy = Math.round(hit.z / 4);
  if (placeWall(G.floor, cx, cy, { barricade: true, hp: 80, dur: Infinity })) {
    G.run.gold -= BARRICADE_COST;
    addMsg(`Barricade built (-${BARRICADE_COST}g)`);
    refreshHud();
  } else {
    addMsg('No room for a barricade there.', 'bad');
  }
}
